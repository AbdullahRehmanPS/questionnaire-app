  ...
  <PageComponent title="Admin">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 text-gray-700">
      ...
      <div class="bg-white shadow-md p-3 text-center flex flex-col animate-fade-in-down order-1 lg:order-2" style="animation-delay: 0.1s">
        ...
        <div v-if="!data.totalQuestionnaires" class="text-8xl font-semibold flex-1 flex items-center justify-center">
          0
        </div>
        <div v-else class="text-8xl font-semibold flex-1 flex items-center justify-center">
      ...
      <div class="bg-white shadow-md p-3 text-center flex flex-col order-2 lg:order-4 animate-fade-in-down" style="animation-delay: 0.2s">
        ...
        <div v-if="!data.totalAnswers" class="text-8xl font-semibold flex-1 flex items-center justify-center">
          0
        </div>
        <div v-else class="text-8xl font-semibold flex-1 flex items-center justify-center">
          {{ data.totalAnswers }}
      ...
      <!--  On Left    -->
      <div class="row-span-2 animate-fade-in-down order-3 lg:order-1 bg-white shadow-md p-4" style="animation-delay: 0.3s">
        <h3 class="text-2xl font-semibold">Latest Questionnaire</h3>
        <div v-if="data.latestQuestionnaires" >
        <div class="p-4">
        ...
        <div class="flex justify-between fixed bottom-4 p-2">
          <router-link :to="{ name: 'QuestionnaireView', params: { id: data.latestQuestionnaires.id } }" class="mr-2 flex py-2 px-4 border border-transparent text-sm rounded-md text-blue-700 hover:bg-blue-700 hover:text-white transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-700">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/>
            </svg>
            Edit Questionnaire
          </router-link>
          <button class="flex py-2 px-4 border border-transparent text-sm rounded-md text-blue-700 hover:bg-blue-700 hover:text-white transition-colors focus:ring-2 focus:ring-offset-2 focus:ring-blue-700">
